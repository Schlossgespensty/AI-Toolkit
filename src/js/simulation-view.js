...
(() => {
  ...
  $('simulationExport').addEventListener('click',async()=>{
    try {
      ...
      if (folder) $('simulationMessage').textContent=`Exported to ${folder}. Put this folder under ucp/modules and enable it with Recorder 0.37.1–0.39.x. See its README; it has not yet had an in-game performance pass.`;
    } catch (error) { $('simulationMessage').textContent=error.message; }
  });
